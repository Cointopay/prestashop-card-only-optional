...
class Cointopay_Direct_Cc_OptionalCallbackModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function initContent()
    {
        ...
        try {
			...
			$merchant_id = Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID');
			$security_code = Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE');
			$user_currency = Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY');
			$selected_currency = (isset($user_currency) && !empty($user_currency)) ? $user_currency : 1;
			$ctpConfig = array(
			  'merchant_id' => $merchant_id,
			  'security_code'=>$security_code,
			  'selected_currency'=>$selected_currency,
			  'user_agent' => 'Cointopay - Prestashop v'._PS_VERSION_.' Extension v'.COINTOPAY_DIRECT_CC_OPTIONAL_PRESTASHOP_EXTENSION_VERSION
			);
            sleep(5);
			\Cointopay_Direct_Cc_Optional\Cointopay_Direct_Cc_Optional::config($ctpConfig);
			$response_ctp = \Cointopay_Direct_Cc_Optional\Merchant\Order::ValidateOrder(array(
            ...
            if (isset($response_ctp)) {
				if(null != $response_ctp->data['Security'] && $response_ctp->data['Security'] != $ConfirmCode)
				{
				   $this->context->smarty->assign(array('text' => $response_ctp->data->Security.'Data mismatch! ConfirmCode doesn\'t match'));
					if (_PS_VERSION_ >= '1.7') {
						$this->setTemplate('module:cointopay_direct_cc_optional/views/templates/front/cointopay_payment_cancel.tpl');
					} else {
				...
				elseif(null != Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID') && $response_ctp->data['MerchantID'] != Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'))
				{
				...
				elseif($response_ctp->data['Status'] != $ctp_order_status && $ctp_order_status_notenough == 0)
				{
				   $this->context->smarty->assign(array('text' => 'We have detected different order status. Your order status is '.$response_ctp->data['Status']));
					...
					if ($ctp_order_status == 'paid' && $ctp_order_status_notenough == 0) {
						...
						$order_status = 'COINTOPAY_DIRECT_CC_OPTIONAL_PNOTENOUGH';
						...
						$order_status = 'COINTOPAY_DIRECT_CC_OPTIONAL_EXPIRED';
						...
						$order_status = 'COINTOPAY_DIRECT_CC_OPTIONAL_WAITING';
					...
					if ($order_status !== false && $order_status == 'PS_OS_PAYMENT') {
						...
						if (_PS_VERSION_ >= '1.7') {
							$this->setTemplate('module:cointopay_direct_cc_optional/views/templates/front/ctp_payment_callback.tpl');
						} else {
							$this->setTemplate('ctp_payment_callback.tpl');
						}
					} elseif ($order_status == 'COINTOPAY_DIRECT_CC_OPTIONAL_PNOTENOUGH') {
						$history = new OrderHistory();
						$history->id_order = $order->id;
						$history->changeIdOrderState((int)Configuration::get($order_status), $order->id);
						$history->addWithemail(true, array(
						...
						if (_PS_VERSION_ >= '1.7') {
							$this->setTemplate('module:cointopay_direct_cc_optional/views/templates/front/ctp_payment_paidnotenough.tpl');
						} else {
						...
						if (_PS_VERSION_ >= '1.7') {
							$this->setTemplate('module:cointopay_direct_cc_optional/views/templates/front/ctp_payment_cancel.tpl');
						} else {
							$this->setTemplate('ctp_payment_cancel.tpl');
						}
					} elseif ($order_status == 'COINTOPAY_DIRECT_CC_OPTIONAL_EXPIRED') {
						$history = new OrderHistory();
						$history->id_order = $order->id;
						$history->changeIdOrderState((int)Configuration::get($order_status), $order->id);
						$history->addWithemail(true, array(
						...
						Tools::redirect($this->context->link->getModuleLink('cointopay_direct_cc_optional', 'cancel'));
					} else {
