...
require_once _PS_MODULE_DIR_ . '/cointopay_direct_cc_optional/vendor/cointopay/init.php';
require_once _PS_MODULE_DIR_ . '/cointopay_direct_cc_optional/vendor/version.php';

class Cointopay_Direct_Cc_Optional extends PaymentModule
{
    public $merchant_id;
    ...
    {
        $this->name = 'cointopay_direct_cc_optional';
        ...
        $config = Configuration::getMultiple(
            array(
                'COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID',
                'COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE',
                'COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY',
                'COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME'
            )
        );

        if (!empty($config['COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'])) {
            $this->merhcant_id = $config['COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'];
        }
        if (!empty($config['COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE'])) {
            $this->security_code = $config['COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE'];
        }
        if (!empty($config['COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'])) {
            $this->crypto_currency = $config['COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'];
        }
    ...
    public function install()
    {
        ...
        if ($order_ctp_pending->add()) {
            copy(
                _PS_ROOT_DIR_ . '/modules/cointopay_direct_cc_optional/views/img/logo.png',
        ...
        Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_PNOTENOUGH', $order_not_enough->id);
        Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_EXPIRED', $order_expired->id);
        Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_INVALID', $order_invalid->id);
        Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_PENDING', $order_ctp_pending->id);
    ...
    {
        $order_not_enough = new OrderState(Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_PNOTENOUGH'));
        $order_state_expired = new OrderState(Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_EXPIRED'));
        $order_state_invalid = new OrderState(Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_INVALID'));
        $order_state_pending = new OrderState(Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_PENDING'));

        return (Configuration::deleteByName('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID') &&
            Configuration::deleteByName('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE') &&
            Configuration::deleteByName('COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME') &&
            Configuration::deleteByName('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY') &&
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            if (!Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID')) {
                $this->postErrors[] = $this->l('Merchant id is required.');
            }

            if (!Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE')) {
                $this->postErrors[] = $this->l('Security Code is required.');
            }

            if (!Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY')) {
                $this->postErrors[] = $this->l('Checkout Currency is required.');
            }

            if (empty($this->postErrors)) {
                $ctpConfig = array(
                    'merchant_id' => Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'),
                    'security_code' => Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE'),
                    'selected_currency' => Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'),
                    'user_agent' => 'Cointopay - Prestashop v' . _PS_VERSION_
                        . ' Extension v' . COINTOPAY_DIRECT_CC_OPTIONAL_PRESTASHOP_EXTENSION_VERSION,
                );

                \Cointopay_Direct_Cc_Optional\Cointopay_Direct_Cc_Optional::config($ctpConfig);

                $merchant = \Cointopay_Direct_Cc_Optional\Cointopay_Direct_Cc_Optional::verifyMerchant();
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME', Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID', Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE', Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE'));
            Configuration::updateValue('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY', Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'));
        }
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME',
                        'required' => true,
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->l('Merchant ID'),
                        'name' => 'COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID',
                    ...
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE',
                    ...
                    array(
                        ...
                        'name' => 'COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY',
                        'id' => 'crypto_currency',
                        'default_value' => (int)Tools::getValue('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'),
                        'required' => true,
                        'options' => array(
                            'query' => $options,
                            'id' => 'id_option',
    ...
    {
        $system_name = Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME');
        $dislay_name = (isset($system_name) && !empty($system_name)) ? $system_name : 'Pay via Visa / Mastercard';

        return array(
            'COINTOPAY_DIRECT_CC_OPTIONAL_DISPLAY_NAME' => $dislay_name,
            'COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID' => Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_MERCHANT_ID'),
            'COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE' => Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_SECURITY_CODE'),
            'COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY' => Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'),
        );
    }
    ...
    private function displayCointopayInformation($renderForm)
    {
        ...
        $this->context->smarty->assign("selected_currency", Configuration::get('COINTOPAY_DIRECT_CC_OPTIONAL_CRYPTO_CURRENCY'));
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (isset($_REQUEST['CustomerReferenceNr'])) {
            ...
            return $this->context->smarty->fetch('module:cointopay_direct_cc_optional/views/templates/hook/ctp_success_callback.tpl');
        }
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $newOption->setCallToActionText($this->displayName)
            ...
            ->setAdditionalInformation(
                $this->context->smarty->fetch('module:cointopay_direct_cc_optional/views/templates/hook/cointopay_intro.tpl')
            )
    ...
    public function hookDisplayAdminOrderMain($order)
    {
        ...
        $OrdCurrency = $this->currencyCode($currency->iso_code);
        $link = new Link();
        $paymentUrl = $link->getModuleLink('cointopay_direct_cc_optional', 'makepayment', array(
